		...
		ContentTypeReporter
{
		...
		try
		{
			...
			else
			{
				for (int i = 0; i < emailRoles.size(); i++)
				{
					...
					UIBoundBoolean input = UIBoundBoolean.make(roleBranch, "mailsender-usersGroup",
							emailBean + ugParams.type + "Ids." + role.getRoleId());
					...
					else
					{
						...
						UIMessage.make(roleBranch, "mailsender-usersGroupLabel",
			...
			UIBranchContainer roleBranch, String[] rolePlural)
	{
		...
		UsersViewParameters usersParams = new UsersViewParameters(
				UsersProducer.VIEW_ID);
		...
		String command = "RcptSelect.showIndividuals(this, '"
				+ usersArea.getFullID() + "', '" + selectLink.getFullID() + "', '"
				+ collapseLink.getFullID() + "');return false";
		UIDecorator onClickDecorator = new UIFreeAttributeDecorator("onclick",
				command);
