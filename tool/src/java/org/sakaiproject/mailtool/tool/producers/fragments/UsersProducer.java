		...
		try
		{
			...
			else
			{
				...
				for (User user : users)
				{
					...
					UIBranchContainer cell = UIBranchContainer.make(tofill, "mailtool-userCol:",
							viewParams.id + "-" + Integer.toString(i));
					UIBoundBoolean checkbox = UIBoundBoolean.make(cell, "mailtool-user",
							"emailBean.newEmail.userIds." + user.getId());
					String displayName = user.getLastName() + ", " + user.getFirstName() + " ("
							+ user.getDisplayId() + ")";
					UIVerbatim label = UIVerbatim.make(cell, "mailtool-userLabel", displayName);
					UILabelTargetDecorator decorator = new UILabelTargetDecorator(checkbox);
					label.decorate(decorator);
					i++;
				}
			}
		}
